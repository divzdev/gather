...
export default function ProfilePage() {
  ...
  const { data: me } = useQuery({ queryKey: ["me"], queryFn: () => authed<Me>("/auth/me") });
        ...
        <div style={{ padding: "20px 28px 80px", maxWidth: 640 }}>
          ...
          <div style={{ ...card, padding: 20, display: "grid", gap: 16 }}>
            {field(
              ...
              <input
                value={name}
                onChange={(event) =>
                  setDraft({ name: event.target.value, density })
                }
                style={input}
              />,
            )}
            ...
            <p style={{ font: "400 12px var(--font-plex-sans)", color: "var(--i4)", margin: "-10px 0 0" }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <button
                style={{ ...pill, opacity: dirty && !save.isPending ? 1 : 0.5 }}
          ...
          <div style={{ ...card, padding: 20, marginTop: 16, display: "grid", gap: 12 }}>
            ...
            <p style={{ font: "400 13px var(--font-plex-sans)", color: "var(--i2)", margin: 0 }}>
              You are <strong>{me?.role || "—"}</strong> in{" "}
              <strong>{me?.org_name || "—"}</strong>. Roles are granted by an owner, so they are not
              editable here.
            </p>
            <p style={{ font: "400 12.5px var(--font-plex-sans)", color: "var(--i3)", margin: 0 }}>
              Theme and accent live in the menu under your avatar, and are remembered per browser
