...
/** Rooms, tracks, formats and days — the skeleton the agenda is drawn on.
 *
 *  The API has had CRUD for all four since the first migration and nothing
 *  reached it, so a newly registered organiser got an event they could not set
 *  up: no rooms to drag a session into, no tracks to file it under. The seed
 *  hid this, because the demo event arrives with all of it already made.
 *
 *  Four lists of the same shape, so they share one component rather than four
 *  near-identical ones.
 */
...
function List({ panel, eventId }: { panel: Panel; eventId: string | null }) {
  ...
  return (
    <section style={{ ...card, padding: 20, marginBottom: 16 }}>
      <h2 style={{ font: "600 16px var(--font-plex-sans)", color: "var(--ik)", margin: "0 0 2px" }}>
        {panel.title}
      </h2>
      <p style={{ font: "400 12.5px var(--font-plex-sans)", color: "var(--i3)", margin: "0 0 14px" }}>
        {panel.blurb}
      </p>
  ...
  return (
    ...
    >
      ...
      <div style={{ overflowY: "auto", padding: "20px 28px 80px" }}>
        <PageHead
          title="Program setup"
          summary="Rooms, tracks, formats and days. The agenda is drawn from these, so it stays empty until they exist."
        />
        {PANELS.map((panel) => (
          <List key={panel.key} panel={panel} eventId={eventId} />
        ))}
      </div>
    </div>
  );
