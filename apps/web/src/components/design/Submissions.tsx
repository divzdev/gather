...
export type SubmissionsData = {
  ...
  /* Hand-bound: promote-to-session, the accept flow's missing second half. */
  ...
  /* Hand-bound: asks the model for near-duplicate submissions (read-only —
   * see the drawer the page renders with the answer). */
  readonly onDupes: (event: React.SyntheticEvent) => void;
  ...
  readonly o: {
    ...
    /** Every other answer on the form. The drawer showed the abstract and the
     *  level and silently dropped the rest, so a form the organiser built
     *  themselves had fields they could not read back on the proposal. */
  ...
  /* Hand-bound: the real team, replacing three fixture names. */
  readonly coordVal: string;
  readonly coordOpts: readonly { readonly v: string; readonly n: React.ReactNode }[];
