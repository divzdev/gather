...
from app.features.ai.gateway import select_adapter
    ...
    async with db.session_factory() as session, session.begin():
        with tenant_scope(org_id=org_id, event_id=event_id):
            ...
            await proposals.record(
                session, proposal, output=output, reasoning=reasoning, model=model, usage=usage
            )


async def _abandon(
    event_id: uuid.UUID, org_id: uuid.UUID, proposal_id: uuid.UUID, reason: str
) -> None:
    async with db.session_factory() as session, session.begin():
        with tenant_scope(org_id=org_id, event_id=event_id):
            proposal = await proposals.get(session, proposal_id)
...
async def answer(
    *,
    ...
    try:
        proposal_id, org = await _open(event_id, org_id, user_id, request.question)
    ...
    plan_system = prompts.load(prompts.ASK_PLAN)
    plan_user = _plan_request(request.history, request.question, today)

    try:
        planning = await llm.complete(
            system=plan_system, user=plan_user, max_tokens=settings.ai_max_tokens
        )
        plan = proposals.parse(planning.text, Plan)
    except ApiError as error:
        await _abandon(event_id, org_id, proposal_id, error.message)
        yield "error", {"message": error.message}
        return
    ...
    if plan.clarify:
        await _close(
            ...
            org_id,
            proposal_id,
            output={"clarify": plan.clarify, "is_stub": planning.is_stub},
            reasoning=plan.clarify,
            model=planning.model,
            usage=planning.usage,
        )
        yield "clarify", {"question": plan.clarify, "is_stub": planning.is_stub}
        return

    if plan.refusal and not plan.queries:
        await _close(
            event_id,
            org_id,
            proposal_id,
            output={"refusal": plan.refusal, "is_stub": planning.is_stub},
            reasoning=plan.refusal,
            model=planning.model,
            usage=planning.usage,
        )
        yield "refusal", {"message": plan.refusal, "is_stub": planning.is_stub}
        return

    ran, results = await _run_plan(event_id, org_id, plan)
